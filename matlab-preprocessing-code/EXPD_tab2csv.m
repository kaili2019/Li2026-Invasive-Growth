...
for i = 1:length(invasive_D3)
    tmp_name = invasive_D3{i,2}(1:end-1);
    if ~contains(tmp_name,surface_D3(:,2))
%         disp(tmp_name)
        rm_idx_D3W(cc) = i;
        cc = cc+1;
    end
...
for i = 1:length(surface_D3)
    tmp_name = surface_D3{i,2}+"w";
    if ~contains(tmp_name,invasive_D3(:,2))
%         disp(tmp_name)
        rm_idx_D3S(cc) = i;
        cc = cc+1;
    end
...
for i = 1:length(invasive_D6)
    tmp_name = invasive_D6{i,2}(1:end-1);
    if ~contains(tmp_name,surface_D6(:,2))
%         disp(tmp_name)
        rm_idx_D6W(cc) = i;
        cc = cc+1;
    end
...
for i = 1:length(surface_D6)
    tmp_name = surface_D6{i,2}+"w";
    if ~contains(tmp_name,invasive_D6(:,2))
%         disp(tmp_name)
        rm_idx_D6S(cc) = i;
        cc = cc+1;
    end
...
for i = 1:ND3
    ...
    % 1. full name or ID
    if contains(invasive_D3{i,2},'796')
        if invasive_D3{i,2}(5:6) == "BD"
            tab_D3{i,1} = "parent";
        else
            tab_D3{i,1} = invasive_D3{i,2}(5:8);
        end
    else
        tab_D3{i,1} = invasive_D3{i,2}(1:5);
    end
    
    % 2. suphide (Y/N)
    if contains(invasive_D3{i,2},'400S')
        tab_D3{i,2} = 'Y';
    else
        tab_D3{i,2} = 'N';
    end
    
    tmp_idx = find(contains(surface_D3(:,2),invasive_D3{i,2}(1:end-1)));
    ...
    if contains(invasive_D3{i,2},'BD50')
        tab_D3{i,7} = "BD50";
    elseif contains(invasive_D3{i,2},'BD75')
        tab_D3{i,7} = "BD75";
    end
    ...
    % 11. Full name
    tab_D3{i,11} = invasive_D3{i,2}(1:end-1);
end
...
% writetable(df_D3,'../raw_data/image_data/slad_expD_day3.csv')
...
for i = 1:ND6
    ...
    % 1. full name or ID
    if contains(invasive_D6{i,2},'796')
        if invasive_D6{i,2}(5:6) == "BD"
            tab_D6{i,1} = "parent";
        else
            tab_D6{i,1} = invasive_D6{i,2}(5:8);
        end
    else
        tab_D6{i,1} = invasive_D6{i,2}(1:5);
    end

    % 2. suphide (Y/N)
    if contains(invasive_D6{i,2},'400S')
        tab_D6{i,2} = 'Y';
    else
        tab_D6{i,2} = 'N';
    end
    
    tmp_idx = find(contains(surface_D6(:,2),invasive_D6{i,2}(1:end-1)));
    ...
    % 7. nutrient: BD50 or BD75
    if contains(invasive_D6{i,2},'BD50')
        tab_D6{i,7} = "BD50";
    elseif contains(invasive_D6{i,2},'BD75')
        tab_D6{i,7} = "BD75";
    end
    ...
    % 11. Full name
    tab_D6{i,11} = invasive_D6{i,2}(1:end-1);
end
...
% writetable(df_D6,'../raw_data/image_data/slad_expD_day6.csv')
